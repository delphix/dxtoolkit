...
for my $engine ( sort (@{$engine_list}) ) {
  ...
  if (defined($importDBHooks)) {
    ...
    my $db_list = Toolkit_helpers::get_dblist_from_filter($type, $group, $host, $dbname, $databases, $groups, undef, undef, undef, undef, $debug);
    if (! defined($db_list)) {
...
=head1 SYNOPSIS
 ...
 dx_ctl_hooks.pl [ -engine|d <delphix identifier> | -all ]  
                 [ -name hook_name ] 
