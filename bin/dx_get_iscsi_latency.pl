...
#
# Program Name : dx_get_iscsi_latency.pl
# Description  : Get iscsi latency
#
...
=head1 SYNOPSIS
 ...
 dx_get_iscsi_latency [ -engine|d <delphix identifier> | -all ] [ -configfile file ] 
                      [-w <warning millisec>] 
                      [-i time_interval] 
                      [-c <critical millisec>] 
                      [-opname operation] 
...
=item B<-w>
Warning level in milliseconds (Integer, Default 10)

=item B<-c>
Critical level in milliseconds (Integer, Default 50)
...
Average iSCSI read and write latency for a last 5 minutes using 1-second sample

 dx_get_iscsi_latency -d DE1
 OK:DE1 disk latency milliseconds 3

Average disk write latency for a last 5 minutes using 1-second sample 

 dx_get_iscsi_latency -d DE1 -opname w 
