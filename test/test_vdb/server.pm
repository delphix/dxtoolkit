...
sub handle_request {    
  ...
  if ($filename && $cgi->request_method eq 'GET') {    	
    readfile($cgi, $filename);
  } elsif ($filename && $cgi->request_method eq 'POST') {        
    checkpost($cgi, $filename);
  } else {        
...
sub checkpost {    
  ...
  my $cgi  = shift;
  my $jsonname = shift;
  open(my $fd, $jsonname) or die("POST Can't open file - $jsonname");
  my @content = <$fd>;
  close($fd);
  
  my @req;
  if ( -f $jsonname . ".req" ) {        
    open(my $fd, $jsonname . ".req") or die("Can't open file - $jsonname");
    @req = <$fd>;
    close($fd);
    my $json = JSON->new();
