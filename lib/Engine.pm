...
sub getJSONResult {
  ...
  if ( $response->is_success ) {
     ...
     if ( $decoded_response =~ /Engine is booting up/ ) {
        ...
        return (\%booting, \%booting, 1);
     } elsif ( $decoded_response =~ /502 Bad Gateway/ ) {
        logger($self->{_debug}, "502 Bad Gateway");
        logger($self->{_debug}, $decoded_response, 2);
        return (\%booting, \%booting, 1);
     } else {
        $result = decode_json($decoded_response);
     }
...
sub uploadupdate60 {
   ...
   my ($r,$r_fmt, $rcode) = $self->getJSONResult("resources/json/system/uploadUpgrade?file=" . basename($filename));
   logger($self->{_debug}, "Already uploaded file size " . $r->{"size"}, 2);
   ...
   logger($self->{_debug}, "Start part: " . $startpart, 2);
   logger($self->{_debug}, "total: " . $total, 2);


   for my $part ($startpart..$noofparts) {
